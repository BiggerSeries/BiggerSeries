...
public class QuartzChunkSection extends ChunkSection {
    ...
    @Nonnull
    @Override
    public BlockState setBlockState(int x, int y, int z, @Nonnull BlockState state, boolean useLocks) {
        BlockState oldBlockState = super.setBlockState(x, y, z, state, useLocks);
        tertiaryWorkQueue.enqueue(() -> blockStateUpdate(oldBlockState, state, new Vector3i(x, y, z)));
