...
public class GuiReactorAutoEjectToggle<T extends Container> extends GuiPartBase<T> implements IHasTooltip, IHasButton {
    ...
    public void drawTooltip(int mouseX, int mouseY) {
        if (this.isHovering(mouseX, mouseY)) {
            if (this.doAutoEject) {
                this.screen.renderTooltip(Arrays.asList(new TranslationTextComponent("tooltip.biggerreactors.buttons.reactor.waste_eject_auto.enabled").getFormattedText().split("\\n")), mouseX, mouseY);
            } else {
                this.screen.renderTooltip(Arrays.asList(new TranslationTextComponent("tooltip.biggerreactors.buttons.reactor.waste_eject_auto.disabled").getFormattedText().split("\\n")), mouseX, mouseY);
            }
        }
    }
