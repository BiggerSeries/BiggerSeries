...
public class GuiTurbineFlowDecreaseButton<T extends Container> extends GuiPartBase<T> implements IHasTooltip, IHasButton {
    ...
    public void drawTooltip(MatrixStack stack, int mouseX, int mouseY) {
        if (this.isHovering(mouseX, mouseY)) {
            this.screen.func_243308_b(stack, Arrays.stream(new TranslationTextComponent("tooltip.biggerreactors.buttons.turbine.flow.decrease").getUnformattedComponentText().split("\\n")).map(StringTextComponent::new).collect(Collectors.toList()), mouseX, mouseY);
        }
    }
