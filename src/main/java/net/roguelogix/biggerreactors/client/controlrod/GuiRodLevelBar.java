...
public class GuiRodLevelBar<T extends Container> extends GuiPartBase<T> implements IHasTooltip {
    ...
    public void drawPart() {
        ...
        // Mask away empty bit.
        GuiRenderHelper.setTextureOffset(0, 82);
        GuiRenderHelper.draw(this.xPos, this.yPos + renderSize, this.screen.getBlitOffset(), this.xSize, this.ySize - renderSize);
    ...
    public void drawTooltip(int mouseX, int mouseY) {
        if (this.isHovering(mouseX, mouseY)) {
            this.screen.renderTooltip(String.format("%.1f%%", this.insertionLevel), mouseX, mouseY);
        }
    }
}