	...
	protected void readFromNBT(CompoundNBT data) {
		for(int i = 0; i < tankNames.length; i++) {
			if(data.hasUniqueId(tankNames[i])) {
				tanks[i] = ReactantStack.createFromNBT(data.getCompound(tankNames[i]));
...
//			if(hasReactant) {
//				ByteBufUtil.writeUtf8(buffer, tanks[i].getName());
...
//
//				if(!ReactorFuelRegistry.isKnown(reactantName)) {
//					// TODO: 6/22/20 LOG
