...
public class CyaniteReprocessorTile extends LockableTileEntity implements ISidedInventory,
    ...
    public boolean isActive() {
        return this.energyStorage.getEnergyStored() > 0
                && this.fluidStorage.getFluidAmount() > 0
                && !itemStorage.getStackInSlot(SLOT_INPUT).isEmpty()
                && itemStorage.getStackInSlot(SLOT_OUTPUT).getCount() < 64;
    }
    ...
    @Override
    public void tick() {
        boolean activityFlag = this.isActive();
        boolean updateFlag = false;
        
        if (this.isActive()) {
            // Consume power and water, and advance timer.
            ++this.workTime;
            this.energyStorage.extractEnergy(1, false);
            this.fluidStorage.drain(1, FluidAction.EXECUTE);
        }
        ...
        if (!this.world.isRemote()) {
            ...
            if (this.isActive()) {
                ...
                if (this.workTime >= this.workTimeTotal) {
                    this.itemStorage.extractItem(SLOT_INPUT, 1, false);
                    this.itemStorage.insertItem(SLOT_OUTPUT, new ItemStack(BlutoniumIngot.INSTANCE, 1), false);
                    this.workTime = 0;
                    System.out.println("FINISHED");
                }
                
                // If item is removed prematurely, decrement work time (similar to vanilla furnaces).
            } else if (this.workTime > 0) {
                --this.workTime;
                System.out.println("DECREMENT");
            }
            
            if (activityFlag != this.isActive()) {
                updateFlag = true;
                this.world.setBlockState(this.pos, this.world.getBlockState(this.pos).with(CyaniteReprocessor.ENABLED, this.isActive()), 3);
            }
        }
